...
% JD2: Thank you for the clarification on that one.
...
% JD2: Good; it's a little nominal, but the last sentence that you added is more in
%      the spirit of what we're looking for in terms of mental model heuristic analysis.
...
When users faced the task of attempting to connect to a wireless network, they each followed the same general steps. They immediately began looking for the WiFi icon in the toolbars of each operating system. The WiFi icon is extremely similar between the different systems, so the users knew to select it to access the wireless network settings. The results for this task, connecting to wifi, can be found in Table \ref{wifiM}, Table \ref{wifiW}, and Table \ref{wifiL}. The average time it took to bring up the list of available wireless networks was less than four seconds on every system. There were no significant differences in the way the users completed the task between the different operating systems.
...
% JD2: Yes, definitely a much better "think-through" than before.
...
% JD2: The Internet similarity is a good point, a clear mental model correlation.
%      Users are now accustomed to hitting a search field when searching on the
%      Internet, so they naturally try to transfer this knowledge to a desktop
%      operating system when trying to find a file.
...
% JD2: *And* it accommodates differences in users, in terms of how they approach
%      running an application.  Some users are menu/browsing oriented; other users
%      are search-oriented.  Again, it's all about knowing what they might be
%      envisioning for a particular task, and providing elements in the user
%      interface that conform to that vision.
...
% JD2: This is quite nicely stated.
